...
- **Live sensing — works in any space, no room capture needed** — the Quest 3's real-time depth sensor feeds a live sonar picture: wall and object returns paint onto the radar as fading traces wherever you are, the pulse scan lights up live geometry points, and a RNG readout under the boresight shows the distance to whatever surface you're looking at (live hit-test). Sim mode fakes the same feed with a ray fan.
...
- **Wrist panel** — look at your left palm (XR) or press P (sim): mode, pulse scan, map toggles, theme, brightness, re-zero north, clear targets/contacts, reset session, and ROOM CAPTURE, which launches Quest Space Setup from inside the HUD (needed only for labeled SCAN contacts — live sensing works without it). Poke buttons with your right index finger.
