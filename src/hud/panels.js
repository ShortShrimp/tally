  ...
  function drawVitals() {
    ...
    const row = (label, value, unit = '', color = t.p) => {
      ...
      if (unit) {
        setFont(c2d, 17);
        c2d.fillStyle = t.dim;
        c2d.fillText(unit, 110 + c2d.measureText(value).width + value.length * 14, y);
      }
      y += 52;
    };
    ...
    c2d.lineWidth = 2;
    c2d.strokeRect(18, y, W - 60, 14);
    c2d.fillStyle = coreColor;
    c2d.fillRect(20, y + 2, (W - 64) * state.core / 100, 10);
    y += 52;
    ...
    setFont(c2d, 30, true);
    c2d.fillStyle = state.mode === 'COMBAT' ? t.accent : t.p;
    c2d.fillText(state.mode, 110, y);
    y += 46;

    setFont(c2d, 20);
    c2d.fillStyle = t.dim;
    c2d.fillText('KILLS', 18, y);
    setFont(c2d, 30, true);
    c2d.fillStyle = t.p;
    c2d.fillText(String(state.kills), 110, y);
    c2d.fillStyle = t.dim;
    setFont(c2d, 18);
    c2d.fillText(`FPS ${Math.round(state.fps)}`, 230, y);
    vitals.commit();
  }
