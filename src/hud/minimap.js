...
export function createMinimap(state, groups) {
  ...
  function draw() {
    ...
    // scanned contacts (hollow squares)
    c2d.lineWidth = 3;
    for (const cnt of (state.contacts || [])) {
      const [x, y] = toMap(cnt.center.x, cnt.center.z);
      if (x * x + y * y > R * R) continue;
      c2d.strokeStyle = cnt.assess === 'WATCH' ? t.accent : t.dim;
      c2d.strokeRect(x - 6, y - 6, 12, 12);
    }

    // designated targets
    for (const tgt of state.targets) {
      const [x, y] = toMap(tgt.pos.x, tgt.pos.z);
      const clampR = Math.min(1, (R - 14) / Math.max(1, Math.hypot(x, y)));
    ...
    c2d.fillText('N', cx + Math.sin(nAng) * (R - 26), cy - Math.cos(nAng) * (R - 26) + 9);
    ...
    c2d.fillStyle = t.p;
    c2d.beginPath();
    c2d.moveTo(0, -15); c2d.lineTo(-10, 11); c2d.lineTo(0, 5); c2d.lineTo(10, 11);
    c2d.closePath();
    c2d.fill();
    c2d.restore();
    ...
    c2d.fillText(`TAC MAP · ${rangeM}M · ${mode}`, cx, W + 30);
    c2d.fillStyle = t.dim;
    setFont(c2d, 20, true);
    c2d.fillText(`CNT ${(state.contacts || []).length} · TGT ${state.targets.length}`, cx, W + 58);
