...
export async function start(sim) {
  ...
  const actions = {
    fire: () => { if (state.phase === 'BOOT') boot.skip(); else targeting.fire(); },
    designateAhead: () => {
      const p = state.camPos.clone().add(new THREE.Vector3(0, 0, -1).applyQuaternion(state.camQuat).multiplyScalar(2.6));
      p.y = Math.max(0.3, p.y);
      targeting.designate(p);
    },
  ...
  if (sim) {
    ...
    xrSession.addEventListener('select', (e) => {
      if (state.phase === 'BOOT') { boot.skip(); return; }
      const handed = e.inputSource.handedness;
      if (handed === 'left') {
        if (latestHit) targeting.designate(latestHit);
        else actions.designateAhead();
      } else {
        targeting.fire();
      }
    });
  ...
  renderer.setAnimationLoop((now, frame) => {
    ...
    // core recharge
    state.core = Math.min(100, state.core + dt * 0.6);
